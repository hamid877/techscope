...
export async function GET(request: Request) {
  try {
    ...
    if (
      !registryRaw ||
      (registryRaw !== 'npm' && registryRaw !== 'pypi')
    ) {
      return NextResponse.json(
        {
          error: 'invalid_request',
          message: 'Missing or invalid registry parameter',
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          metrics:
            result.metricsBreakdown?.map((metric) => ({
              metric: metric.metric,
              percentile: metric.percentile,
              weight: metric.weight,
              status:
                metric.percentile !== null ? 'success' : 'unavailable',
            })) ?? [],
        },
        { status: 200 }
      );
